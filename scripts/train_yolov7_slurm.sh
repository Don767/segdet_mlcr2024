...
docker run --gpus all -e CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES --rm --ipc host \
  ...
  segdet bash -c "mim install mmcv==2.1.0 mmdet==3.3.0 && python3 segdet/models/yolov7/yolo_train.py --workers 8 --device $CUDA_VISIBLE_DEVICES --batch-size 128 --data config/coco.yaml --img 640 640 --cfg config/config_yolov7.yaml --weights yolov7_training.pt --name yolov7-base --hyp config/hyp_scratch_yolov7.yaml"
